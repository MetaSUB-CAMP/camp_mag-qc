...
1. Clone repo from `Github <https://github.com/MetaSUB-CAMP/camp_mag-qc>`_. 

2. Set up the conda environment (contains, Snakemake) using ``configs/conda/mag_qc.yaml``. 

3. Download the database dependencies for GTDB-Tk and CheckM.
...
::
    ...
    -d /path/to/camp_mag-qc/test_out \
    -s /path/to/camp_mag-qc/test_data/samples.csv \
    -p /path/to/camp_mag-qc/test_data/parameters.yaml \
    -r /path/to/camp_mag-qc/test_data/resources.yaml \
    --cores 20
...
**Input**: ``/path/to/samples.csv`` provided by the user.
    * Please note that the ``samples.csv`` generated by the CAMP binning module needs to be manually pruned so that there is only a single column called 'mags' indicating the absolute path to the directory containing the MAG FastA files.
...
::
    ...
    python /path/to/camp_mag-qc/workflow/mag_qc.py cleanup \
        -d /path/to/work/dir \
        -s /path/to/samples.csv
...
::
    ...
    python /path/to/camp_mag-qc/workflow/mag_qc.py commands cmds.txt
